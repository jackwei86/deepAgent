...
- 移除 `start_tray.vbs` 启动方式，统一使用 `start_tray.bat`（启动瞬间黑框闪现不到一秒，之后托盘无窗口运行）

### 新增

- **撤销/还原（checkpoint 回退命令）**：新工具 `撤销还原`——用户说"撤销抠图，还原到美颜那一步"即可把处理状态回退到指定历史步骤（追加式，历史产物不删除，还原内容与检查点逐字节一致）；验证 9/9 通过；并支持"还原到最开始(原图)"——task_id 留空时自动取本轮对话第一张上传的原始图作为还原目标
