...
> v1.1.0 之上的修复与增强：本地图片过滤器（图片数据不出本机）、撤销还原完善、环境级补丁入库。

### 新增

- **本地图片过滤器（全局 Filter）**：图片数据不出本机——消息中的图片（base64 内联 / /api/v1/files 引用 / 顶层 files 元数据三种形态）自动截留保存到 `data/webui/uploads/`，发给 LLM 的只有文字占位"[用户上传的图片已保存: <路径>]"；模型名含 vision 时自动跳过
- **撤销还原支持"还原到最开始(原图)"**：撤销工具 task_id 留空时自动取本轮对话第一张上传的原图作为还原目标（还原内容与原图逐字节一致）
- **环境级补丁入库** `patches/open-webui-0.6.43/`：登录表单补丁（用户名+去邮箱校验）与侧边栏入口注入的前端文件、原始备份与一键 apply/restore 脚本
- **验证脚本**：`docs/verify_image_filter.py`（过滤+标记解析联合 5/5）、`docs/debug_image_400.py`（DeepSeek 图片边界测试）

### 修复

- **多轮对话累积图片导致 DeepSeek 400、回复卡住**：根因为历史图片每轮全量重发（base64 膨胀后累计数十 MB）触发 DeepSeek 请求拒绝；由本地图片过滤器根治——图片不再出网
- **撤销还原工具报错** `execute_tool_task() got an unexpected keyword argument 'inputs'`：移除多余参数
- **侧边栏入口位置**：从底部调整为"工作空间"项之后（按链接地址 href 精确定位，兼容英文界面），并加分割线

### 变更

- 登录名由 `admin@deepagent.local` 简化为 `admin`；登录表单标签改"用户名"
- 素材库页面主题自动跟随聊天页深/浅色设置（同源共享 theme），配色对齐 Open WebUI
- 返回按钮文案改为"← 返回对话"
- 防幻觉加固：模型档案注入系统提示词（必须真实调用工具、严禁虚构结果）

### 文档

- `docs/疑问.md`：第 1 节补充图片流量实测结论与 400 根因；新增第 7 节（LLM 返回后管线固定性 vs LangGraph）、第 8 节（工具调用原理确认）、第 9 节（CLI vs pybind11 评估）
- `docs/任务信息JSON格式.md`：schema 1.1（parent_task_id/pipeline_id/restore 类型）
- `docs/任务链与版本树-一期实施.md`、`docs/任务链与版本树-二期LangGraph方案.md`（暂不实施）
