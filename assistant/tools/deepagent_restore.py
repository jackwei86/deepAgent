...
class Tools:
    ...
    async def restore_checkpoint(
        ...
        两种还原目标：
        ① 还原到某个历史步骤：task_id 填该步的任务单编号（出现在此前工具返回的
           "任务单：`编号`"信息中，格式如 20260911-105524-a3d6）；
        ② 还原到最开始（原始上传的图）：task_id 留空即可，工具自动取本轮对话
           第一张上传的原图作为还原目标。用户说"还原到最开始"、"撤销所有处理"、
           "回到原图"时用这种方式。
        还原采用追加式：历史任务与产物保留不删除，还原后会生成一个内容等于
        还原目标的新任务，后续处理自动基于它继续。
