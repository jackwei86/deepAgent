...
class Tools:
    ...
    async def video_beauty(
        ...
        """对视频执行逐帧美颜处理（磨皮/降噪/可选美白），输出美化后的视频并提供播放链接。
        处理耗时与视频长度成正比，过程中实时汇报进度百分比。
        素材来源（二选一）：① 用户消息中附带的视频；② 素材库文件。
        重要："素材库"、"收藏夹"、"当前目录"指当前用户的素材库目录（video 子目录存视频）——
        用户提到这些词并给出文件名时，必须把文件名传入 file_name 参数直接调用，
        不要以"没有看到视频"为由要求用户重新上传；若素材库和附件中都没有该文件，
        工具会返回明确的提示。
        处理链：若处理的素材本身是此前任务的结果，系统会自动登记派生关系（parent/链ID），在素材库"任务产物"页可查看完整处理链版本树，并支持从任意历史结果继续处理。
        示例："美化素材库里的 clip.mp4，强度0.5"。

        :param strength: 美颜强度 0.0-1.0，默认 0.7；"轻一点"用 0.4，"强一点"用 0.9。
        :param whiten: 美白程度 0.0-1.0，默认 0 关闭。
        :param denoise: 是否附加降噪，默认 true。
        :param file_name: 素材库视频文件名（如 clip.mp4 或 video/clip.mp4）。消息已附带视频时留空。
        """
        ...
        return await tool_impl.execute_tool_task(
            ...
            description=f"对视频逐帧美颜，强度 {strength}，美白 {whiten}，降噪 {denoise}",
